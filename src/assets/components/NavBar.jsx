...
export default function NavBar() {
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1 overflow-x-hidden">
            {navItems.map(({ path, label, icon }) => (
              ...
              >
                ...
                <span
                  className={`font-medium transition-all duration-300 ${isCollapsed ? "lg:hidden" : ""}`}
                >
                  {label}
                </span>
                {isActive(path) && (
          ...
          <div className="border-t border-white/10 p-4">
            ...
            {currentUser && (
              <div
                className={`mt-3 px-3 py-2 rounded-lg bg-white/5 transition-all duration-300 ${isCollapsed ? "lg:hidden" : ""}`}
              >
      ...
      <div
        className={`hidden lg:block ${isCollapsed ? "lg:w-20" : "lg:w-64"}`}
      />
    </>
  );
}
