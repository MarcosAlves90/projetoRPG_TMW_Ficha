...
export default function StatCard({
  ...
  return (
    <div
      className={`card ${colorClass} hover-lift ${compact ? "p-2" : ""} ${className}`}
    >
      <div
        className={`flex items-center justify-between ${compact ? "mb-1" : "mb-3"}`}
      >
        <span
          className={`text-gray-400 font-semibold uppercase tracking-wide ${compact ? "text-xs" : "text-xs"}`}
        >
          {label}
        </span>
        {icon && (
          <span className={`opacity-80 ${compact ? "" : "text-2xl"}`}>
            {icon}
          </span>
        )}
      </div>
      <p className={`font-bold ${compact ? "text-2xl" : "text-4xl"}`}>
        {value}
      </p>
    </div>
  );
