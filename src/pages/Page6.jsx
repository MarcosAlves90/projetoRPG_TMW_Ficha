...
export default function Page6() {
  ...
  const handleInputChange = useCallback(
    (e) => {
      ...
      setLocalItem((prevItem) => {
        ...
        if (name === "type")
          setShowQuantityInput(/^\s*(consumível|consumíveis)\s*$/i.test(value));
        return updatedItem;
      });
  ...
  const deleteItem = useCallback(() => {
    saveItems(
      userData.itemsArray.filter((item) => item.id !== selectedItem.id),
    );
    closeModal();
  ...
  const pasteItemCode = useCallback(
    async (e) => {
      e.preventDefault();
      try {
  ...
  const filteredItems = useMemo(() => {
    return (userData.itemsArray || []).filter((item) => {
      ...
      const itemCategories = item.type
        .split(",")
        .map((category) => category.trim())
        .filter((category) => category !== "");
      const matchesCategories =
        activeCategories.length === 0 ||
        activeCategories.some((category) => itemCategories.includes(category));
  ...
  const memoizedItems = useMemo(
    () =>
      filteredItems.map((item) => (
        ...
        >
          <img
            className={`image ${!item.image ? "image-placeholder" : ""}`}
            src={item.image || placeHolderImage}
            alt="Item"
          />
          <p className="title">{item.title.toUpperCase()}</p>
          {/^\s*(consumível|consumíveis)\s*$/i.test(item.type) && (
            <p className="title qty">{(item.quantity || "").toUpperCase()}</p>
          )}
        </div>
      )),
  ...
  const searchByCategory = useCallback(
    (category) => {
      const modifiedCategories = activeCategories.includes(category)
        ? activeCategories.filter((d) => d !== category)
            ...
            <div className="rightBox">
              ...
              <div className="inputsSequence display-flex-center w-100">
                <TextField
                  ...
                  onChange={handleInputChange}
                  label="Categorias"
                  placeholder="Categorias do item, divididas por vírgula."
                />
                {showQuantityInput && (
                  <TextField
                    variant="filled"
              ...
              <div className="boxButtons display-flex-center w-100">
                ...
                <button className="button copy" onClick={copyItemCode}>
      ...
      <main className="mainCommon page-6">
        ...
        <section className="tag-cloud display-flex-center">
          <span className={"tag qty"}>
            ...
            {`${userData.itemsArray ? userData.itemsArray.length : 0} Itens`}
          </span>
        ...
        <article className="boxInventory">{memoizedItems}</article>
      </main>
    </>
  );
}
