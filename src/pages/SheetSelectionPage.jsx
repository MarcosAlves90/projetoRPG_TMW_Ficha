...
export default function SheetSelectionPage() {
  ...
  function addSheet() {
    if (
      !newSheetName.trim() ||
      sheets.some((sheet) => sheet.nome === newSheetName)
    ) {
      console.error(
        "O nome da ficha está vazio ou é igual ao nome de outra ficha.",
      );
      return;
    }
  ...
  function deleteSheet(sheetCode) {
    if (
      window.confirm(
        "Quer mesmo deletar essa ficha? Esse processo é irreversível.",
      )
    ) {
  ...
  function switchSheet(sheetCode) {
    ...
    if (selectedSheet) {
      let updatedSheets = sheets.filter(
        (sheet) => sheet.sheetCode !== sheetCode,
      );
        ...
        {loading ? (
          ...
          <>
            ...
            <div className={"sheet-list"}>
              <div className={`sheet active display-flex-center`}>
                <p className={"p-name"}>
                  {userData.nome || userData.sheetCode}
                </p>
              </div>
              {sheets.map((sheet) => (
                <div
                  className={`sheet display-flex-center`}
                  key={sheet.sheetCode}
                >
                  <p
                    className={"p-name"}
                    onClick={() => switchSheet(sheet.sheetCode)}
                  >
                    {sheet.nome || sheet.sheetCode}
                  </p>
                  <i
                    className="bi bi-trash3"
                    onClick={() => deleteSheet(sheet.sheetCode)}
                  />
                </div>
              ))}
            </div>
