...
export default function Page4() {
    ...
    const uniqueDomains = useMemo(() => {
        const domains = (userData.skillsArray || [])
            ...
            .flatMap(skill => skill.domain.split(',')
    ...
    const memoizedItems = useMemo(() => (filteredSkills || []).map((skill) => (
        <div key={skill.id} className="skill skill-list" onClick={() => openModal(skill)}>
            <img className={`image ${!skill.image ? "image-placeholder" : ""}`} src={skill.image || placeHolderImage} alt="Skill" />
    ...
    return (
        <>
            <ReactModal isOpen={modalIsOpen} onRequestClose={closeModal} className="popup skill" overlayClassName="popupOverlay" bodyOpenClassName="no-scroll">
                {localItem && (
                    <>
                        ...
                        <div className="rightBox">
                            ...
                            <textarea className="popup-content textarea" value={localItem.content} name="content" onChange={handleInputChange} placeholder="Descrição da skill" />
                            <TextField value={localItem.domain} fullWidth name="domain" className="popupInput" onChange={handleInputChange} variant={"filled"} label="Domínios" />
                            {window.innerWidth <= 991 && (
                                    <TextField
                                    value={localItem.image}
                                    fullWidth
            ...
            <main className="mainCommon page-4" ref={pasteRef}>
                ...
                <section className="tag-cloud display-flex-center">
                    ...
                    {uniqueDomains.map((domain) => {
                        ...
                        const displayDomain = domain.length > 40 ? `${domain.slice(0, 30)}...` : domain;
                        return (
                            <span
                                key={domain}
                                className={`tag 
